...
class Blockchain {
    ...
    requestMessageOwnershipVerification(address) {
        return new Promise((resolve) => {
            let signedMessage =`${address}:${new Date().getTime().toString().slice(0,-3)}:starRegistry`;
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if((currentTime - messageTime) > (5 * 60)) {
                ...
                reject("block More than 5 minutes have elapsed since message creation");
            }
        });
    }
     ...
     */
     ...
     getStarsByWalletAddress (address) {
        ...
        let stars = [];
        return new Promise((resolve, reject) => {
        //slice to ignore genesis
        self.chain.slice(1).forEach(block => {
        block.getBData().then(objectData => {
        if (objectData.owner === address){
        stars.push(objectData.star);
        }
        });
        })
        resolve(stars);
        ...
        return new Promise(async (resolve, reject) => {
            ...
            for (const block of self.chain) {
                ...
                if (!isValid || block.previousBlockHash !== prevBlockHash) {
                    errorLog.push({
                        block,
                        error: 'Invalid check previousBlockHash'
                    })
                }
